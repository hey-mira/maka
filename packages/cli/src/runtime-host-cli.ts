...
function parseProfileCommand(argv: string[]): RuntimeHostCliCommand {
  ...
  let operator: RuntimeHostOperatorCommand | undefined;
  let expectedRootId: string | undefined;
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (argument === '--operator-command') {
      try {
        operator = decodeRuntimeHostOperatorCommand(JSON.parse(parsed));
      } catch {
  ...
  if (wslDistribution && !operator) {
    return error('--wsl-distribution requires --operator-command');
  }
  if (!wslDistribution && operator) {
