...
export interface RuntimeHostNodeOperatorCommand {
  readonly kind: 'node';
  readonly platform: RuntimeHostOperatorPlatform;
...
export function createRuntimeHostOperatorCommand(input: {
  readonly platform: RuntimeHostOperatorPlatform;
  readonly nodePath: string;
  readonly modulePath: string;
}): RuntimeHostNodeOperatorCommand {
...
export function runtimeHostManagedOperatorCommand(
  deployment: {
  ...
  return createRuntimeHostOperatorCommand({
    ...
    modulePath: paths.join(deployment.deploymentRoot, 'operator.mjs'),
  });
}
...
export function decodeRuntimeHostOperatorCommand(value: unknown): RuntimeHostOperatorCommand {
  ...
  if (record.kind === 'legacy_posix_executable') {
    ...
    if (record.platform !== 'posix') {
      throw new Error('Legacy Runtime Host operator platform is invalid');
    }
