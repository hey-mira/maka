...
#[napi]
pub fn windows_task_converge(root_id: String, target: String, command: Vec<String>) -> Result<()> {
...
#[napi]
pub fn windows_task_verify(root_id: String, target: String, command: Vec<String>) -> Result<()> {
    let target = require_target(&root_id, &target)?;
    validate_command(&command).map_err(|_| invalid("Windows lifecycle command is invalid"))?;
    ...
    verify_registered_definition(&task, &root_id, target, &command, &context.user)
        .map_err(native_error)
}
...
#[napi]
pub fn windows_task_activate(root_id: String, target: String) -> Result<()> {
    let target = require_target(&root_id, &target)?;
    let context = scheduler().map_err(native_error)?;
    let name = task_name(&root_id, target);
    let task =
    ...
    unsafe {
        if matches!(target, Target::Host)
            && task.State().map_err(native_error)? != TASK_STATE_RUNNING
        {
...
#[napi]
pub fn windows_task_retire(root_id: String, target: String) -> Result<()> {
    let target = require_target(&root_id, &target)?;
    let context = scheduler().map_err(native_error)?;
    let name = task_name(&root_id, target);
    if let Some(task) =
...
fn converge_task(
    ...
    let desired = normalized_definition(&context.service, root_id, target, command, &context.user)?;
    let empty = VARIANT::default();
...
fn verify_registered_definition(
    ...
    unsafe {
        let expected_command = task_action_command(target, command)?;
...
fn assert_owned(
    ...
    // SAFETY: task is a live thread-local COM interface and owns the returned BSTR.
    let xml = unsafe { task.Xml()? }.to_string();
    if !xml.contains(&ownership_marker(root_id, target)) {
...
fn render_task_xml(
    ...
    let trigger = match target {
        ...
        Target::Reconciliation => concat!(
            "<RegistrationTrigger><Delay>PT15M</Delay><Enabled>true</Enabled></RegistrationTrigger>",
    ...
    let action_command = task_action_command(target, command)?;
...
fn task_action_command(target: Target, command: &[String]) -> windows::core::Result<Vec<String>> {
    if matches!(target, Target::Reconciliation) {
        return Ok(command.to_vec());
    }
    if command.len() < 4
        || !Path::new(&command[1]).is_absolute()
        || command[2] != "runtime-host"
        || command[3] != "serve"
    {
        return Err(invalid_windows_request());
    }
    let supervisor = Path::new(&command[1]).with_file_name("runtime-host-windows-supervisor.js");
    let supervisor = supervisor
        .to_str()
        .ok_or_else(invalid_windows_request)?
        .to_owned();
    let projected = [command[..1].to_vec(), vec![supervisor], command.to_vec()].concat();
